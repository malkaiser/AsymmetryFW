...
void CLoop::Fill(double weight, int z_sample, const std::string& sampleName) {
  ...
  // Opp sign passed iso electronplus 1 prong
  if (ql==qtau && tight_electron  && ql < 0 && tau_0_n_charged_tracks == 1 && elec_0_p4->Pt() > 27 && one_electron && n_taus==1 && electron_id && no_bjets && tauBDT){ // electron channel
