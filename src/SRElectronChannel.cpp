...
void CLoop::Fill(double weight, int z_sample, const std::string& sampleName) {
  ...
  if (ql!=qtau && one_electron && elec_0_p4->Pt() > 27 && n_taus==1 && electron_id && no_bjets && !tight_electron && tauBDT){ // muon channel
    ...
    lep_ptContainer.Fill(elec_0_p4->Pt(),weight,notFullCutsVector); // muon transverse momentum
