...
void CLoop::Fill(double weight, int z_sample, const std::string& sampleName) {
  ...
  if (sampleName.substr(0,4)!="data"){
      weight = weight*elec_0_NOMINAL_EleEffSF_Isolation_TightLLH_d0z0_v13_FCLoose*elec_0_NOMINAL_EleEffSF_Isolation_TightLLH_d0z0_v13_FCTight
  ...
  bool tight_electron{elec_0_iso_FCTight == 1};
  bool tauBDT{tau_0_ele_bdt_score_trans_retuned > 0.15};
  float ql;
  float qtau;
  ...
  if (ql==qtau && one_electron && n_taus==1 && electron_id && no_bjets && !tight_electron && tauBDT){ // electron channel
    ...
    // CUTS
    if (tau_0_n_charged_tracks == 3 || tau_0_n_charged_tracks == 1 && tau_0_jet_rnn_score_trans >= 0.15) {cuts[0] = 1;} // 1-pronged tau RNN
    if (tau_0_n_charged_tracks == 1 || (tau_0_n_charged_tracks == 3 && tau_0_jet_rnn_score_trans >= 0.25)) {cuts[1] = 1;} // 3-pronged tau RNN
    ...
    lepisoContainer.Fill(elec_0_iso_FCTight,weight,notFullCutsVector);
...
void CLoop::FillTree(double weight, int z_sample, const std::string& sampleName) {
  ...
  float ql;
  float qtau;
  bool failed_tight_electron{elec_0_iso_FCTight == 1}; // failed isolation
  ...
  if (ql!=qtau && one_electron && n_taus==1 && electron_id && failed_tight_electron && tauBDT){ // electron channel
    ...
    std::vector<int> cuts={0,0,0,0,0}; // tutorial
    // CUTS
    if (tau_0_n_charged_tracks == 1 && tau_0_jet_rnn_score_trans >= 0.15) {cuts[0] = 1;} // 1-pronged tau RNN
    else if (tau_0_n_charged_tracks == 3 && tau_0_jet_rnn_score_trans >= 0.25) {cuts[0] = 1;} // 3-pronged tau RNN
